...
Based on this equation the error can be written as 
$$
s_{\mu} = \sqrt{ \sum_{i=1}^N (X_i - \mu)^2 / (N-1)}.
$$
...
N<-50
x_N<-sample(x,size=N,replace=TRUE)
s_mu<-sqrt(sum((x_N-mu)^2)/(N-1))
s_mu
s<-sqrt(sum((x_N-mean(x_N))^2)/(N-1))
s
sd(x_N)
```
