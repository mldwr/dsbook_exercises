...
another plot for the p-value of a pollster-pollster combination :\
...
The probability of two children being siblings dying of SIDS is 1 in 850.000 or 1.18e-06.
$$
\mbox{Pr}(\mbox{second case of SIDS}) = \mbox{Pr}(\mbox{second case of SIDS} \mid \mbox{first case of SIDS}) * \mbox{Pr}(\mbox{first case of SIDS}) = 1/100 \cdot 1/8500 = 1/850000
$$

In the following snippet the numerical calculation is shown. 
```{r}
options(scipen = 1)
PrA <- 1/8500 
PrBA <- 1/100
...
However, other authors assume different probabilities. For example, in the article^[https://doi.org/10.1111/j.1365-3016.2004.00560.x; Ray Hill, (2004) Multiple sudden infant deaths – coincidence or beyond coincidence?, Pediatric and
Perinatal Epidemiology, 18, 320-326,  http://www2.geog.ucl.ac.uk/~mdisney/teaching/GEOGG121/mc_and_revision/mc_bayes_revision_2017.pdf] the author concluded that the probability of two SIDS cases is $\mbox{Pr}(\mbox{second case of SIDS}) = 1/1300 \cdot 57/13000 = 3.37\cdot 10^{-06}$. Nevertheless, this probability is in the same order of magnitude. \
In the same study the author deduced a probability for two murders to be $\mbox{Pr}(\mbox{second case of murder}) = 1/21700 \cdot 176/21700 = 3.74\cdot 10^{-07}$. \
Therefore, the probability of a double SIDS ($3.37\cdot 10^{-06}$) is roughly ten times more likely than the probability of a double child murder ($3.74\cdot 10^{-07}$).
...
So the question states the hypothesis, that the jury were interpreting the probability "1 in 73 million" (1 in $73\cdot 10^{6}$) as the chances of Sally Clark being innocent, not as the probability of two infants dying from SIDS.
Therefore, the misconception was that the probability of Sally Clark being innocent is $\frac{1}{73\cdot 10^{6}}$. \
From this it follows, that the jury assumed the probability Sally Clark being guilty is $1-\frac{1}{73\cdot 10^{6}}$ (assuming for the probability of hypothesis $H$ and evidence $E$: $\mbox{Pr}(H|D) = 1-\mbox{Pr}(\mbox{not} H|E)$). \
How can this hypothesis be modeled? 

Let be the hypothesis $H$, that the mother is a the murderer: "mother murderer" =  "_a mother is a son-murdering psychopath_". Let the event $E$ be the two dead children "dead children" =
"_two of her children are found dead with no evidence of physical harm_". Then one can write the the Bayes equation as follows.


$$
\mbox{Pr}(\mbox{mother murderer} \mid \mbox{dead children}) = \mbox{Pr}(\mbox{dead children} \mid \mbox{mother murderer}) \cdot \frac{\mbox{Pr}(\mbox{mother murderer})}{\mbox{Pr}(\mbox{dead children})}
$$
The correlation of the two events is modeled by the elaborated version of Bayes Theorem, where the expression $not H$ denotes the complement of $H$, in other word $H$ is the true hypothesis and $\mbox{not } H$ is the false hypothesis.
The expression $\mbox{Pr}(H \mid E)$ is read as what is the probability of a hypothesis $H$ being true, if $E$ has already occurred, where $E$ denotes the evidence, data or the facts. 
...
$$
In other words, one can write as an interpretation for Bayes Theorem: $\mbox{Probability} = \frac{\mbox{desired event}}{\mbox{all possibilities}}$^[https://betterexplained.com/articles/an-intuitive-and-short-explanation-of-bayes-theorem/].
...
3. $\mbox{Pr}(\mbox{dead children})= 1/8500 \cdot 1/100= 1/850000$ and \
2. $\mbox{Pr}(\mbox{dead children} \mid \mbox{mother murderer}) = 1/2$.\

$$
\mbox{Pr}(\mbox{mother murderer} \mid \mbox{dead children}) =  1/2 \cdot  \frac{1/1000000}{1/850000} = 1/2 \cdot \frac{85}{100} = 1/2 \cdot \frac{17}{20} = 0.5\cdot 0.85  = 0.425
$$
The results tells us that the probability of _two of her children are found dead with no evidence of physical harm_ given that _a mother is a son-murdering psychopath_ results in 0.425. \

Therefore, the probability that the mother is not a murderer is: $\mbox{Pr}(\mbox{mother not murderer} \mid \mbox{dead children}) = 1-0.425 = 0.575$, that is rather small compared to the previous figure of 1 in $73\cdot 10^{6}$.

Another variant for the calculation from exercises 2 would be the following: \
1. $\mbox{Pr}(\mbox{mother murderer}) = 1/21700 \cdot 176/21700 = 3.74\cdot 10^{-07}$, \
3. $\mbox{Pr}(\mbox{dead children})= 1/1300 \cdot 57/13000 = 3.37\cdot 10^{-06}$ and \
2. $\mbox{Pr}(\mbox{dead children} \mid \mbox{mother murderer}) = 1/2$.\

$$
\mbox{Pr}(\mbox{mother murderer} \mid \mbox{dead children}) =  1/2 \cdot  \frac{1/21700 \cdot 176/21700}{1/1300 \cdot 57/13000} = 0.5\cdot 0.11  = 0.055
$$
Again, the probability that the mother is *not* a murderer is: $\mbox{Pr}(\mbox{mother not murderer} \mid \mbox{dead children}) = 1-0.055 = 0.945$, that is close to one, i.e. a high probability that Sally Clark might be innocent. 

A flaw in this calculation, however,  is the assumption of the probabilities of two dead children and two cases of SIDS is set to equal: $\mbox{Pr}(\mbox{two dead children})= \mbox{Pr}(\mbox{two cases of SIDS})$ (see exercises 2).

A more general view on this question would be to take the two competing hypothesis explicitly into account^[https://www2.mpia-hd.mpg.de/homes/calj/statistical_methods_ss2013/homework/h02_sally_clark_cot_death.pdf, C.A.L. Bailer-Jones: Statistical Methods SS2013]. \
Since the actual values of the probabilities are hard to recover, it might make more sense to look at the ratios of the two hypothesis: \
$H_1=M$: probability of double murder,\
$H_2=S$: probability of double SIDS and \
$E$: event or evidence of two dead children.\

$$
\frac{\mbox{Pr(S|E)}}{\mbox{Pr(M|E)}} = \frac{\mbox{Pr(E|S)}\cdot \mbox{Pr(S)}}{\mbox{Pr(E|M)}\cdot \mbox{Pr(M)}}
$$
Here, the probabilities of all dead children of $\mbox{Pr(D)}$ cancel each other out. The two terms $\mbox{Pr(D|S)}$ and $\mbox{Pr(D|M)}$ describe the likelihood that given two children were found dead due to SIDS $S$ or murder $M$ how likely it is that they are dead D. Under this assumption the probabilities trivially equal to 1. \

Finally, the relevant terms are revealed as being the ratio of double SIDS to double murder.
$$
\frac{\mbox{Pr(S|E)}}{\mbox{Pr(M|E)}} = \frac{\mbox{Pr(S)}}{\mbox{Pr(M)}}
$$
As already concluded in exercise 2 and in the calculation above, the ratio of $\mbox{Pr(S)}$ to $\mbox{Pr(M)}$ is 1 to 9 

$$
\frac{\mbox{Pr(S|E)}}{\mbox{Pr(M|E)}} = \frac{\mbox{Pr(S)}}{\mbox{Pr(M)}} = \frac{1/1300 \cdot 57/13000}{1/21700 \cdot 176/21700} = \frac{57/1690}{176/47089} = \frac{57\cdot 47089}{1690\cdot 176} = \frac{2684073}{297440} = 9.02
$$
Criticism on the data basis and model of the latter calculations were also raised^[N.Sesardic, Sudden Infant Death or Murder? A Royal Confusion About Probabilities, Brit. J. Phil. Sci. 58 (2007), 299–329, https://www.ln.edu.hk/philoso/staff/sesardic/SIDS.pdf], which state quite the contrary that actually the probability of murder is 125 times higher than for SIDS.
