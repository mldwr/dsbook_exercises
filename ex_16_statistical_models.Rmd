...
The correlation of the two events is modeled by the elaborated version of Bayes Theorem, where the expression $A^c$ denotes the complement of $A$.
$$
\mbox{Pr}(A \mid B)  =  \frac{\mbox{Pr}(B \mid A)\mbox{Pr}(A)}{\mbox{Pr}(B)} = \frac{\mbox{Pr}(B \mid A)\mbox{Pr}(A)}{\mbox{Pr}(B \mid A)\mbox{Pr}(A) + \mbox{Pr}(B \mid A^c)\mbox{Pr}(A^c)}
$$
Applying Bayes Theorem to the problem at hand leads to the following expression.
$$
\mbox{Pr}(\mbox{first case of SIDS} \mid \mbox{second case of SIDS}) = \frac{\mbox{Pr}(\mbox{first case of SIDS})\ \mbox{Pr}(\mbox{second case of SIDS} \mid \mbox{first case of SIDS})}{\mbox{Pr}(\mbox{second case of SIDS})}
$$
The values for the probabilities are $\mbox{Pr}(\mbox{first case of SIDS}) = 1/8500$ and for $\mbox{Pr}(\mbox{second case of SIDS} \mid \mbox{first case of SIDS}) = 1/100$.

For the probability of the denominator further terms have to be computed $\mbox{Pr}(\mbox{second case of SIDS}) = \mbox{Pr}(\mbox{first})\ \mbox{Pr}(\mbox{second} \mid \mbox{first}) + (1- \mbox{Pr}(\mbox{first})) \cdot (1-\mbox{Pr}(\mbox{second} \mid \mbox{first}))$. 

For the complement terms following values will be used $(1- \mbox{Pr}(\mbox{first case of SIDS})) = 1-1/8500=8499/8500$ and $(1-\mbox{Pr}(\mbox{second case of SIDS} \mid \mbox{first case of SIDS}))=1-1/100=99/100$.

Putting all numbers together one gets the following result: 
$$
\mbox{Pr}(\mbox{first} \mid \mbox{second}) = \frac{1/8500  \cdot 1/100}{1/8500  \cdot 1/100 + 8499/8500  \cdot 99/100} = \frac{1/850000}{841402/850000} = \frac{1}{841402}
$$

```{r}
options(scipen = 1)
PrAPrBA <- 1/8500 * 1/100
PrAPrBAc <- 99/100 * 8499/8500
PrB <- PrAPrBA + PrAPrBAc
PrAPrBA/PrB
```
...
First, a few abbreviations are introduced in order to safe some space in the equations:
_a mother is a son-murdering psychopath_ = _mother psychopath_ and
_two of her children are found dead with no evidence of physical harm_ = _no physical harm_.

$$
\mbox{Pr}(\mbox{mother psychopath} \mid \mbox{no physical harm}) = \frac{\mbox{Pr}(\mbox{mother psychopath})\ \mbox{Pr}(\mbox{no physical harm} \mid \mbox{mother psychopath})}{\mbox{Pr}(\mbox{no physical harm})}
$$
...
$\mbox{Pr}(\mbox{mother psychopath}) = 1/1000000$ \
$\mbox{Pr}(\mbox{no physical harm} \mid \mbox{mother psychopath}) = 1/2$ \
$\mbox{Pr}(\mbox{no physical harm})=1/850000$. \

$$
\mbox{Pr}(\mbox{mother psychopath} \mid \mbox{no physical harm}) = \frac{1/1000000 \cdot 1/2}{1/850000} = \frac{17}{40} = 0.425
$$
The results tells us that the probability of _two of her children are found dead with no evidence of physical harm_ and the mother being _a mother is a son-murdering psychopath_ results in 0.425.
This finding is in strong contradiction to the probability of 1 to 73 million. 
...
Answer b. \
This question is the result of questions 1 and 4. 
