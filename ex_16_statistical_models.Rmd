...
d_hat <- polls %>% 
  ...
  pull(d_hat)
p_hat <- (d_hat+1)/2
polls_sum<-polls %>% group_by(pollster) %>% summarize(se = 2 * sqrt(p_hat * (1-p_hat) / median(samplesize)))
polls_sum
polls_sum$se[1]-polls_sum$se[2]
```
...
polls_sum<-polls %>% group_by(pollster) %>% 
  filter(n() >= 6) %>%
  summarize(se = 2 * sqrt(p_hat * (1-p_hat) / median(samplesize))) %>% arrange(se)
...
polls %>% arrange(spread)
polls_mx$se[2]-polls_mx$se[1]
```
...
Opposite to the expectation value the standard error does not obey linear transformations. The following auxiliary equation will be used, first $\mbox{SD}[X] = +\sqrt{\mbox{Var}(X)}$ and second $\mbox{Var}(X-Y) = \mbox{Var}(X)+\mbox{Var}(Y)$, which follows from the variance of the sum of independent random variables \ 
It also holds true that $\mbox{Var}(\bar{X}) = \sqrt{\mbox{SE}[X]}= \sigma/\sqrt{N}$.
In addition, it is used that $\mbox{SE}[X] = \mbox{SD}[X]/\sqrt{N}$ the standard error can be approximated by the standard deviation.\
Putting it all together, the previous formulas can be written as $\mbox{Var}(X)=\sigma^2$ for variance, $\mbox{SD}[X]=\sigma$ for standard deviation and $\mbox{SE}[X]=\sigma/\sqrt{N}$ for standard error.  
...
```{r, message=FALSE}
data(polls_us_election_2016)
polls <- polls_us_election_2016 %>% filter(enddate >= "2016-10-15" & state == "U.S.") %>% mutate(spread = rawpoll_clinton/100 - rawpoll_trump/100) %>% filter(pollster=="Rasmussen Reports/Pulse Opinion Research")
real<-48.2
b1<-polls$rawpoll_clinton %>% mean()-real
d<-polls$spread %>% mean()
d
b1
d+b1
s<-polls$spread %>% sd()/16
s
```
