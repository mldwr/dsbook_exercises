...
polls<-polls_us_election_2016 %>% 
  filter(enddate >= "2016-10-15" &
           state == "U.S.") %>% 
  mutate(spread = rawpoll_clinton/100 - rawpoll_trump/100) %>% group_by(pollster) %>% 
  filter(n() >= 6) %>%
...
Answer d, because $b_i$ defines the bias for pollster $i$ the question in regard if there is a pollster bias. Only for the last option the appropriate question is asked.
...
$$
Y_{1,2} = d + b_1 + \varepsilon_{1,2}
$$
  
$$
Y_{2,1} = d + b_2 + \varepsilon_{2,1}
$$
