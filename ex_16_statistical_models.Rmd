...
Opposite to the expectation value the standard error does not obey the xxx law. The following auxiliary equation will be used. First $\mbox{SD}[X] = +\sqrt{\mbox{Var}(X)}$ and second $\mbox{Var}(X-Y) = \mbox{Var}(X)+\mbox{Var}(Y)$.

$$
SE[\bar{Y}_{2} - \bar{Y}_1] = \sqrt{SE[\bar{Y}_2]+SE[\bar{Y}_1]} = \sqrt{\frac{\sigma_2^2}{\sqrt{N_2}}+\frac{\sigma_1^2}{\sqrt{N_1}}}
$$
