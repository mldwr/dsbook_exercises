...
Opposite to the expectation value the standard error does not obey linear transformations. The following auxiliary equation will be used, first $\mbox{SD}[X] = +\sqrt{\mbox{Var}(X)}$ and second $\mbox{Var}(X-Y) = \mbox{Var}(X)+\mbox{Var}(Y)$. In addition, it is used that $\mbox{SE}[X] = \mbox{SD}[X]/\sqrt{N}$ the standard error can be approximated by the standard deviation.\
