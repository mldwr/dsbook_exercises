...
data(polls_us_election_2016)
polls <- polls_us_election_2016 %>% filter(enddate >= "2016-10-15" & state == "U.S.") %>% mutate(spread = rawpoll_clinton/100 - rawpoll_trump/100)  %>% select(pollster, samplesize,spread)
sigma <-polls %>% group_by(pollster) %>% summarise(sdev=round(sd(spread),4),n=n()) %>% drop_na()
sigma %>% mutate(se=round(sdev/n,4)) %>% select(pollster,sdev,n,se) %>% arrange(sdev)
```
