...
In chapter 16.2 "*Data-driven models*" it is explained that: "A problem is that we don't know $\sigma$. But theory tells us that we can estimate the urn model $\sigma$ with the _sample standard deviation_ defined as $s = \sqrt{ \sum_{i=1}^N (X_i - \bar{X})^2 / (N-1)}$.". Here, the sample standard deviation value $s$ can be computed with the function `sd()`. 
...
This method of computing the p-value has been applied also in chapter 15.10.5 "*Confidence intervals for the odds ratio*", in which the confidence interval was computed along with the p-value. 
