...
polls_merge<-polls_merge %>% mutate(c.pollster=paste(a.pollster,"_",b.pollster)) %>% mutate(significant=as.factor(ifelse(tstat<0.05,"yes","no"))) %>% mutate(null_value=as.factor(ifelse(CI1<=0 & CI2>=0,"yes","no")))
polls_merge
plot(polls_merge)
...
ggplot(polls_merge,aes(a.pollster, EXEY, color=null_value))+
  geom_point() +
  coord_flip() + 
