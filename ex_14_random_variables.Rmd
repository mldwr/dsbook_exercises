...
$$
where $\mbox{E}[S]$ is the expected value and $\mbox{SE}[S]$ the standard error of $S$. The uppercase letter $Z$ is the normalized random variable $S$ with values between 0 and 1. In chapter 13 question 6 it was found that the probability can be computed with the R function `pnorm(z)`$=\mbox{Pr}\left(Z <  z\right)$. Therefore the z-transform can be computed by the inverse `pnorm` function which is `qnorm`: `qnorm(p)`$=z$.
...
It follows that the bank earns 600 million dollars with an error of plus/minus 10 million dollars. 
