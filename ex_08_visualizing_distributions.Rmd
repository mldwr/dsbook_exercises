...
Therefore,  comparing the top areas of the whiskers of Americas and Asia, the part of Asia towers above the part of Americas significantly. From visual inspection the actual range of Americas is roughly 300 million and the range of Asia is more than 1000 million. \
...
data(gapminder)
tab <- gapminder %>% filter(year == 2010) %>% group_by(continent) %>% select(continent, population)  
tab %>% summarise((max(population)-min(population))/10^6)
```
