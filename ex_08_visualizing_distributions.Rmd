...
As can be seen from the table the proportion of the states in the North Central region are round about 25%.
...
heights %>% 
  ...
  geom_segment(aes(x = 50, y = 35, xend = 65.5, yend = 35), color="blue",size=0.1) +
...
```{r}
m <- ggplot(murders, aes(x = population/10^6))
m <- m + geom_density() + scale_x_log10()
p <- ggplot_build(m)
pdat<- p$data[[1]] %>% select(x,y) 
pdat
#head(p$data[[1]], 3)
#p$data[[1]]
#str(p$data)

pdat %>% ggplot(aes(x=x,y=y)) + #scale_x_log10() +
                  geom_line()
```
...
murders %>% ggplot(aes(x=population/10^6)) + 
  ...
  #xlab("Population in millions") +
  #geom_segment(aes(x = 10, y = 0.0, xend = 10, yend = 0.6), color="blue",size=0.3)
```
...
Answer d. \
The x-axis of the first plot is linear, because of the equal tick distances in steps of 10. The x-axis of the other charts are plotted with logarithmic distance, with increasing distances from 1, 3, 7, 20. The first plot is not undersmoothed, as suggested by answer c, due to the lack of multiple peaks. The curvature of plot one is smooth and shows only one peak. 
