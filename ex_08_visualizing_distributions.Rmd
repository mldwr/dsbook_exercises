...
```{r}
m <- ggplot(murders, aes(x = population/10^6)) + geom_density() + scale_x_continuous(trans="log2")
p <- ggplot_build(m)
pdat<- p$data[[1]] %>% select(x,y) #%>% mutate(x=x+12) %>% mutate(x=x/16)
pdat
#head(p$data[[1]], 3)
#p$data[[1]]
#str(p)
...
```{r}
murders %>% ggplot(aes(x=population/10^6)) + geom_density() + scale_x_continuous(trans="log2") #+
  #xlab("Population in millions") +
  #geom_segment(aes(x = 10, y = 0.0, xend = 10, yend = 0.6), color="blue",size=0.3)
a<- seq(1,30,1)
b<-log2(a)
