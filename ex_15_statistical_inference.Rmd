...
poll<-poll %>% mutate(se_hat=sqrt(d_hat * (1 - d_hat) / samplesize))
poll<-poll %>% mutate(lower=d_hat - 1.96 * se_hat, upper=d_hat + 1.96 * se_hat)
poll<-poll %>% select(-samplesize)
poll %>% mutate(hit=ifelse(lower<= d & upper >= d,1,0)) #%>% #select(pollster,lower,upper,hit)
```
...
poll<-poll %>% mutate(se_hat=sqrt(d_hat * (1 - d_hat) / samplesize))
poll<-poll %>% mutate(lower=d_hat - 1.96 * se_hat, upper=d_hat + 1.96 * se_hat)
poll<-poll %>% mutate(hit=ifelse(lower<= d & upper >= d,1,0)) %>%  mutate(pollnr=1:nrow(poll))
poll <- mutate(poll, d_inside = ifelse(hit, "Yes", "No") )
mean(poll$hit)
ggplot(poll, aes(pollnr, d, ymin=lower, ymax=upper, col = d_inside)) + 
  #geom_point()+
  geom_errorbar() + 
  coord_flip() + 
