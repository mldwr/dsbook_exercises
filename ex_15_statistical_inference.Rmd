...
First, a few auxiliary formulas should be recalled. The variance of a random variable is defined as the expected value of the squared deviation from the mean: $\mbox{Var}(X) = \sigma^2=\mbox{E}[(X-\mbox{E}[X])^2]=\mbox{E}[X^2]-\mbox{E}[X]^2$. \
Second, the connection between standard deviation and variance is $\mbox{SD}(X) = +\sqrt{\mbox{Var}(X)} = \sigma$. \

The result from $\mbox{SE}(\bar{X})$ follows directly from the previous definition in the answer of question 4. However, the standard error of a constant value, here $\mbox{SE}(1)=0$ always equals to be 0, because there cannot be any deviation from a sample size of one element (the mean of the constant itself). \

In terms of equations the standard error $\mbox{SE}[X] = \mbox{SD}[X]/\sqrt{N}$ can be computed by utilizing the definition of the variance in terms of the expectation value as 

$$
\mbox{SE}(\bar{X}) = \mbox{SD}(\bar{X}) = \sqrt{\mbox{Var}(\bar{X})}=\sqrt{\mbox{E}(\bar{X^2})-\mbox{E}(\bar{X})^2}.
$$ 
For a constant value if follows $\mbox{SE}(1)^2=\mbox{E}(1^2)-\mbox{E}(1)^2 = 1-1=0$. 
...
ggplot(poll, aes(pollster, d_hat, ymin=lower, ymax=upper, col = d_inside)) + 
  geom_point()+
  #geom_errorbar() + 
  coord_flip() + 
  geom_hline(yintercept = d)
...
Answer c, because here the estimated p-value is 0.052 with a confidence interval of 0.9937118 to 6.9342286. As stated in the question, the confidence interval does not include the "null value" 1, therefore the value, which is formulated by the "Null Hypothesis" the odds ratio. \
