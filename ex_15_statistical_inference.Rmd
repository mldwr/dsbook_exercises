...
ggplot(df,aes(sample=scale(errors))) + geom_qq() + geom_abline()
```
...
Assuming the mean equals to $p$, the standard deviation to the standard error and the measured quantity $\bar{X}$ one can utilize the `pnorm` function to compute the probability. \
The calculation results in a probability of 16% that is $\bar{X}>0.5$.
...
In other words the question is asking: \
What is the probability that the difference of the true $p$-value to the estimate $\bar{X}$ i.e. the error $| \bar{X} - p|$, is smaller than 0.01, i.e. within an error range of 1%.
$$
