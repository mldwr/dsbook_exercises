...
library(kableExtra)
mat <- cbind(c(" Q5 "," Q6 ", " Q7 "), c(" Monte Carlo simulation "," theoretical prediction ", " estimate of theoretical prediction "), c(" 0.04949939 "," 0.04974937 ", " 0.04983974 "))
colnames(mat) <- c("Question","Method", "Value")
...
The hint in this exercise that in the next chapter the reason for the large amount of pollsters missing to cover the final p- and d-value is due to several biases in the system. These will be discussed in the section of Bayes theorem in chapter 16.8.3 "Mathematical representations of models".
...
````
```{r}
chisquare<-seq(0,4,0.05)
pval<-pchisq(chisquare, df =  1, lower.tail = FALSE)
df<-data.frame(chisquare,pval)
df
pchisq(3.0573, df =  1, lower.tail = FALSE)
ggplot(df,aes(x=chisquare,y=pval))+
  geom_point() +
  geom_line(color="blue") +
  ylab("p-value")+
  xlab(expression(chi^{2})) +
  geom_segment(aes(x = 0, y = 0.08038, xend = 3.0573+0.5, yend = 0.08038), color="red",size=0.3) +
  geom_segment(aes(x = 3.0573, y = 0, xend = 3.0573, yend = 0.08038+0.1), color="red",size=0.3)
```
