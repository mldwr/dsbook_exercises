...
Second, the connection between standard deviation and variance is $\mbox{SD}(X) = +\sqrt{\mbox{Var}(X)}$. \
...
In terms of equations the standard error can be approximated by utilizing the definition of the variance in terms of the expectation value as 

$$
\mbox{SE}(\bar{X}) \approx \mbox{SD}(\bar{X}) = \sqrt{\mbox{Var}(\bar{X})}=\sqrt{\mbox{E}(\bar{X^2})-\mbox{E}(\bar{X})^2}.
$$ 
