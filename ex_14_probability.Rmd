...
First create the ball structure.
```{r}
balls <- rep(c("cyan", "magenta", "yellow"), times = c(3,5,7))
```

Then we make random draw with `sample` and repeate the random draw with `replicate` for a large number of times (nr_perms), then we evalute the result with `prop.table`.
```{r}
set.seed(1986)
B <- 10000
...
Alternativley, we can untilise the parameters of the function `sample` to create a large number of draws directly. The result is the same. 
```{r}
set.seed(1986)
B <- 10000
...
Threfore, you cannot distinguish if the event $A$ has taken place before or after event $B$.
...
celtic_win <- replicate(B,sum(sample(c(1,0), 7, replace = TRUE, prob = c(0.6, 0.4))))
